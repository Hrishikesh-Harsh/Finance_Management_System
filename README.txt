...
With the help of our software, a user can:
...
All operations can be performed with the aid of simple, elegantly-designed menus. A user-friendly experience has been ensured, along with a secure as well as non-compromisable database system. The use of SHA-256 encryption for storing and managing passwords makes the system extremely reliable and secure.
...
The following Python-3 libraries need to be installed beforehand (if not preinstalled already) externally for the software to function appropriately. One may use "pip install <package_name>" in the terminal to do the same. Kindly ensure that they are importably installed before attempting to run the program. 
...
If the program still reports that AdminInfo.csv is absent, try relocating/moving the AdminInfo.csv to the default location of the command prompt or terminal (for instance, C:\User\Desktop>).

------------------------
3.2: Input Instructions
------------------------

We have not performed explicit error-trapping (it is not within the purview of this course nor is it a requirement of this project), therefore the inputs must be entered sensibly. In menus, the responses need to be integers within the range specified by the menu. Other inputs are to be entered exactly as specified in the program. Nonsensical inputs will not be handled very sensibly by the program.

Sample Data:

Sample data has been added into the database through the script file that must be run before executing the python program. These sample data consist of users, their accounts, and loans/investments, etc that the users might have on those accounts. If one wishes to use the sample data, one will require the passwords of the users (these passwords are internally stored in the database using the SHA-256 encryption algorithm, so it is secure). The sample users and their passwords are listed below:
...
|                  | its BranchID.                                                   |
+------------------+-----------------------------------------------------------------+
| View Investment  | Displays all the investment schemes that have been made         |
| Schemes          | available on the software for the user's knowledge.             |
+------------------+-----------------------------------------------------------------+
